...
class Text2MotionDataset(data.Dataset):
    ...
    def __init__(self, opt, mean, std, split_file, times=1, w_vectorizer=None, eval_mode=False):
        ...
        self.use_unlabeled_clean = opt.use_unlabeled_clean
        if opt.is_train and opt.noisy_ratio > 0:
            t_noise_lower = opt.t_noise_lower
            t_noise_upper = opt.t_noise_upper

            n_total = len(name_list)
            n_noises = int(n_total * opt.noisy_ratio)
            print()
            ...
            for i, (_, data) in enumerate(data_dict.items()):
                ...
                data['motion'] = diffusion.q_sample(data['motion'].unsqueeze(0), _t_noise)
    ...
    def __getitem__(self, item):
        ...
        return caption, motion, m_length, clean_mask
