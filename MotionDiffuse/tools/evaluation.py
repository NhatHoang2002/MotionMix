...
def evaluation(log_file):
    with open(log_file, 'w') as f:
        ...
        for replication in range(replication_times):
            ...
            if (replication + 1) % 2 == 0 or replication + 1 == replication_times:
                mean_dict = {}
...
if __name__ == '__main__':
    ...
    args.checkpoint_path = "/".join(args.model_ckpt.split("/")[:-2])
    opt_path = os.path.join(args.checkpoint_path, "opt.txt")
    dataset_opt_path = opt_path

    if args.eval_mode == "debug":
        num_sample_limit = 1000
        mm_num_samples = 0
        mm_num_repeats = 0
        mm_num_times = 0
        diversity_times = 300
    ...
    try:
        opt.which_epoch = args.model_ckpt.split("/")[-1].split(".")[0]
    except:
    ...
    log_file = os.path.join(args.checkpoint_path,
                    f"t2m_evaluation_{args.eval_mode}_{opt.which_epoch}_{args.mask_at_time}_{datetime.now().strftime('%m%d%Y_%H_%M%S')}.log")
    evaluation(log_file)
