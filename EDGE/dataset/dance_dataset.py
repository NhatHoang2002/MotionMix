...
class AISTPPDataset(Dataset):
    def __init__(
        ...
        **kwargs
        ...
        data['clean_masks'] = [True for _ in data['pos']]
        noisy_ratio = kwargs.get('noisy_ratio', 0.0)
        self.use_unlabeled_clean = kwargs.get('use_unlabeled_clean', False)
        if self.train and noisy_ratio > 0.0:
            assert len(data['q']) == len(data['pos'])

            t_noise_lower = kwargs.get('t_noise_lower', 0)
            t_noise_upper = kwargs.get('t_noise_upper', 0)

            print(f"Adding noise of range {t_noise_lower} to {t_noise_upper} to {noisy_ratio} of the data")
            print(f"Hiding label for clean data: {self.use_unlabeled_clean}")

            n_total = len(data['q'])
            aug_indices = random.sample(range(n_total), int(n_total * noisy_ratio))

            for i in aug_indices:
                data['clean_masks'][i] = False
                _t_noise = torch.randint(t_noise_lower, t_noise_upper, (1,)).to(diffusion.sqrt_alphas_cumprod.get_device())

                _q = torch.from_numpy(data['q'][i]).float().to(diffusion.sqrt_alphas_cumprod.get_device())
                _q = diffusion.q_sample(_q.unsqueeze(0), _t_noise)
                data['q'][i] = _q.squeeze().detach().clone().cpu().numpy()

                _pos = torch.from_numpy(data['pos'][i]).float().to(diffusion.sqrt_alphas_cumprod.get_device())
                _pos = diffusion.q_sample(_pos.unsqueeze(0), _t_noise)
                data['pos'][i] = _pos.squeeze().detach().clone().cpu().numpy()
        else:
            print("Use all clean data from AIST++...")
    ...
    def __getitem__(self, idx):
        ...
        clean_mask = self.data['clean_masks'][idx]
        if clean_mask and self.use_unlabeled_clean:
            feature = torch.zeros_like(feature)
        return self.data["pose"][idx], feature, filename_, self.data["wavs"][idx], clean_mask
