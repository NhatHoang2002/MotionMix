...
class AMASSDataset(Dataset):
    def __init__(
        ...
        **kwargs
        ...
        pickle_name = "processed_amass_data.pkl"
        ...
        if not force_reload and pickle_name in os.listdir(backup_path):
            ...
            data = self.load_amass()  # Call this last
        ...
        print("Use all clean data from AMASS...")
    ...
    def load_amass(self):
        db = joblib.load('/path/to/amass.pth.tar') # reference to TEACH's code
        ...
        all_names = []
        all_wavs = []
        for data_i, data in enumerate(db):
            q = data["poses"]
            pos = data["trans"]
            ...
            for i in range(0, len(q), 100):
                if i+150 > len(q):
                    break
                if not self.pfc_check(q[i:i+150], pos[i:i+150]):
                    continue

            all_pos.append(pos[i:i+150])
            all_q.append(q[i:i+150])
            all_names.append('/path/to/random/feat')
            all_wavs.append('<none>')
