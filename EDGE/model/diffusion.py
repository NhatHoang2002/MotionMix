...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        self.mask_at_time = mask_at_time
    ...
    def long_ddim_sample(self, shape, cond, **kwargs):
        ...
        for time, time_next, weight in tqdm(time_pairs, desc = 'sampling loop time step'):
            if not self.training and time < self.mask_at_time:
                cond = torch.zeros_like(cond, device=device)
            time_cond = torch.full((batch,), time, device=device, dtype=torch.long)
    ...
    def p_losses(self, x_start, cond, t, clean_mask):
        noise = torch.randn_like(x_start)
    ...
    def loss(self, x, cond, clean_mask=None, t_noise_pivot=0, t_override=None):
        batch_size = len(x)
        if t_override is None:
            t = torch.randint(0, self.n_timestep, (batch_size,), device=x.device).long()
            if t_noise_pivot > 0:
                t[clean_mask] = torch.randint(0, t_noise_pivot, (t[clean_mask].shape[0],), device=x.device).long()
                t[~clean_mask] = torch.randint(t_noise_pivot, self.n_timestep, (t[~clean_mask].shape[0],), device=x.device).long()
        else:
            t = torch.full((batch_size,), t_override, device=x.device).long()
        return self.p_losses(x, cond, t, clean_mask)

    def forward(self, x, cond, clean_mask=None, t_noise_pivot=0, t_override=None):
        return self.loss(x, cond, clean_mask, t_noise_pivot, t_override)
