...
class Text2MotionDatasetV2(data.Dataset):
    def __init__(self, opt, mean, std, split_file, w_vectorizer, args=None):
        self.opt = opt
        self.w_vectorizer = w_vectorizer
        self.max_length = 20
        self.pointer = 0
        ...
        self.use_unlabeled_clean = args.use_unlabeled_clean
        if opt.split == "train" and args.noisy_ratio > 0.0:
            t_noise_lower = args.t_noise_lower
            t_noise_upper = args.t_noise_upper

            n_total = len(name_list)
            n_noises = int(n_total * args.noisy_ratio)
            ...
            aug_indices = random.sample(range(n_total), n_noises)
            assert len(aug_indices) == n_noises
            diffusion = create_gaussian_diffusion(args)

            for i, (_, data) in tqdm(enumerate(data_dict.items()), total=n_total):
                if i not in aug_indices:
                    continue

                data['clean_mask'] = False
                data['motion'] = torch.from_numpy(data['motion']).float()

                # "Z normalization"
                data['motion'] = (data['motion'] - mean) / std
                t = torch.randint(t_noise_lower, t_noise_upper, (1,))

                data['motion'] = diffusion.q_sample(data['motion'].unsqueeze(0), t)
                data['motion'] = data['motion'].squeeze().detach().numpy()

                # "Z un-normalization"
                data['motion'] = data['motion'] * std + mean
    ...
    def __getitem__(self, item):
        idx = self.pointer + item
        data = self.data_dict[self.name_list[idx]]
        motion, m_length, text_list, clean_mask = data['motion'], data['length'], data['text'], data['clean_mask']
        # Randomly select a caption
        text_data = random.choice(text_list)
        caption, tokens = text_data['caption'], text_data['tokens']
        ...
        # print(tokens)
        return word_embeddings, pos_one_hots, caption, sent_len, motion, m_length, '_'.join(tokens), clean_mask
...
class HumanML3D(data.Dataset):
    def __init__(self, args, mode, datapath='./dataset/humanml_opt.txt', split="train", **kwargs):
        self.mode = mode
...
# A wrapper class for t2m original dataset for MDM purposes
class KIT(HumanML3D):
    def __init__(self, args, mode, datapath='./dataset/kit_opt.txt', split="train", **kwargs):
        super(KIT, self).__init__(args, mode, datapath, split, **kwargs)