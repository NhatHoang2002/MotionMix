...
class UESTC(Dataset):
    ...
    def __init__(self, args, datapath="dataset/uestc", method_name="vibe", view="all", **kargs):
        ...
        total_num_actions = 41
        self._clean_masks = [True for _ in self._pose]
        if self.split == "train" and args.noisy_ratio > 0:
            ...
            if args.use_unlabeled_clean:
                self._actions = [41 if self._clean_masks[i] else self._actions[i] for i in range(len(self._actions))]
    ...
    def __getitem__(self, index):
        ...
        output = super().__getitem__(data_index)
