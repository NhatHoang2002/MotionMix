...
from utils.parser_util import evaluation_parser, get_cond_mode
...
def evaluate(args, model, diffusion, data):
    ...
    args.cond_mode = get_cond_mode(args)
    ...
    metricname = "evaluation_results_iter{}_samp{}_mask{}_scale{}_a2m_{}.yaml".format(iter, args.num_samples, args.mask_at_time, scale, datetime.now().strftime("%m%d%Y_%H%M%S"))
...
def main():
    ...
    data_loader = get_dataset_loader(name=args.dataset, num_frames=60, split='test', batch_size=args.batch_size, args=args)

    print("creating model and diffusion...")
    model, diffusion = create_model_and_diffusion(args, data_loader)
    model.mask_at_time = args.mask_at_time
